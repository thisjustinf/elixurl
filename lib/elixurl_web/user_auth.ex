defmodule ElixurlWeb.UserAuth do
  use ElixurlWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_elixurl_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      ElixurlWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the `current_scope`:
      ...
      defmodule ElixurlWeb.PageLive do
        use ElixurlWeb, :live_view

        on_mount {ElixurlWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ElixurlWeb.UserAuth, :require_authenticated}] do
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      ElixurlWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
